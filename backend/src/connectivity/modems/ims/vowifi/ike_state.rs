...
/// Owned access facts fixed at the beginning of one IKE connection. They can
/// come from a per-SIM override without mutating or leaking the carrier catalog.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct IkeAccessConfig {
    pub ip_stack: String,
...
impl IkeStateMachine {
    ...
    pub fn new_with_dh_group(
        ...
        Self::new_with_dh_group_and_access(
            profile,
            IkeAccessConfig::from_profile(profile),
            initiator_spi,
            initiator_nonce,
            initiator_public_dh,
            initiator_dh_group,
    ...
    pub fn new_with_dh_group_and_access(
        profile: &'static CarrierProfile,
        access: IkeAccessConfig,
        initiator_spi: u64,
        initiator_nonce: Vec<u8>,
    ...
    fn build_auth_init_payloads(
        ...
        if self.profile.ikev2.include_epdg_idr {
            let responder_identity = self.access.apn.as_deref().unwrap_or(&self.access.epdg_host);
            payloads.insert(
                1,
