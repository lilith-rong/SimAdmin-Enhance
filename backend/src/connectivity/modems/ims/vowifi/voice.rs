...
//! `crate::connectivity::core::voice`. This re-export preserves the established VoWiFi module
//! API while live VoWiFi transport code is migrated incrementally.

#![allow(dead_code)]

pub use crate::connectivity::core::voice::*;
...
pub fn voice_params(profile: &'static CarrierProfile) -> VoiceParams {
    VoiceParams {
        ...
        codec_policies: profile
            ...
            .iter()
            .filter_map(|policy| {
                AudioCodec::from_token(policy.codec).map(|codec| AudioCodecPolicy {
                    codec,
