...
use crate::connectivity::core::ims_video::{
    build_av_sdp, build_video_offer, VideoMediaDescription,
};
...
use crate::platform::config::ImsVideoConfig;
...
pub struct VolteVoiceCall {
    ...
    /// Shared IMS video configuration (codec / payload type / fmtp).
    ims_video: ImsVideoConfig,
    /// Whether video is permitted for this call (feature-gated).
    video_enabled: bool,
...
impl VolteVoiceCall {
    ...
    /// Enable video for this call. `volte_enabled` gates whether video can be
    /// added mid-call; `local_video_media` is the (distinct) RTP endpoint the
    /// device relays the video stream on.
    pub fn with_vilte(
        mut self,
        ims_video: &ImsVideoConfig,
    ...
    /// Whether video may be added to this call (feature-enabled and a
    /// local video media endpoint is configured).
    pub fn video_available(&self) -> bool {
        self.video_enabled && self.local_video_media.is_some()
    }
    ...
    pub fn upgrade_to_video(&mut self) -> Result<MediaReoffer, MediaSwitchError> {
        ...
        let video: VideoMediaDescription = build_video_offer(
            &self.ims_video.codec,
            self.ims_video.video_payload_type,
            &self.ims_video.h264_fmtp,
            video_media.port,
        );
