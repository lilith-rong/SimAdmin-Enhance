...
macro_rules! verr {
    ($code:expr) => {
        $crate::connectivity::modems::softstack::volte::errors::VolteError::new($code)
    };
    ($code:expr, $detail:expr) => {
        $crate::connectivity::modems::softstack::volte::errors::VolteError::with_detail($code, $detail)
    };
}
