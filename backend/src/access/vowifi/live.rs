...
fn parse_sip_status(frame: &[u8]) -> Result<u16, LiveStageError> {
    ...
    crate::ims::sip_frame::parse_status(frame).map_err(|err| live_stage_error(err.code()))
}
...
fn sip_complete_frame_len(buffer: &[u8]) -> Option<usize> {
    crate::ims::sip_frame::complete_frame_len(buffer)
}

fn find_sip_header_end(buffer: &[u8]) -> Option<usize> {
    crate::ims::sip_frame::find_header_end(buffer)
}
...
fn aka_digest_password(
    ...
    let material = crate::ims::digest_aka::AkaMaterial {
        res: &aka.res,
        ck: &aka.ck,
        ik: &aka.ik,
    };
    crate::ims::digest_aka::aka_digest_password(algorithm, &material).map_err(|err| {
...
fn hmac_md5(key: &[u8], data: &[u8]) -> [u8; 16] {
    ...
    crate::ims::digest_aka::hmac_md5(key, data)
}
...
mod tests {
    ...
    fn akav2_md5_digest_uses_res_ik_ck_without_serializing_values() {
        ...
        let aka = crate::access::vowifi::qmi_uim::UsimAkaApduResult {
            res: vec![0x22; 8],
            ck: vec![0x33; 16],
            ik: vec![0x44; 16],
            auts: None,
