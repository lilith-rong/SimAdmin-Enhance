...
impl ProfileStore {
    ...
    pub fn migrate_legacy_profiles_file(&self, path: &std::path::Path) -> Result<usize, String> {
        ...
        let legacy = crate::infra::config::parse_external_vowifi_profiles(&content);
        let mut migrated = 0;
        for entry in legacy {
    ...
    pub fn list_as_external(
        &self,
    ) -> Result<Vec<crate::infra::config::ExternalVowifiProfile>, String> {
        let mut out = self
            .list()?
            .into_iter()
            .map(|stored| crate::infra::config::ExternalVowifiProfile {
    ...
    pub fn save_external(
        &self,
        entry: &crate::infra::config::ExternalVowifiProfile,
    ) -> Result<(), String> {
...
"#,
    ...
    fn external_shaped_edit_preserves_the_tuned_register_policy() {
        ...
        store
            .save_external(&crate::infra::config::ExternalVowifiProfile {
