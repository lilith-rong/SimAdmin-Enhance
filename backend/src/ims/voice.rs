//! Shared clean-room IMS voice state machine, SDP and RTP codecs.
//!
...
/// Build a transport-neutral MO audio offer.
...
/// Build an SDP answer by intersecting a received offer with local parameters.
...
impl VoiceCallStateMachine {
    /// Construct from transport-agnostic voice params.
...
pub fn build_dry_run_voice_snapshot_with_params(params: VoiceParams) -> VoiceRuntimePublicState {
    let mut machine = VoiceCallStateMachine::with_params(params);
...
pub fn select_voice_leg_with_params(
    params: &VoiceParams,
    vowifi_ready: bool,
    carrier_usb_audio_available: bool,
) -> VoiceLegKind {
    if params.vowifi_enabled && vowifi_ready {
        return VoiceLegKind::Vowifi;
    }
    if params.carrier_fallback_enabled && carrier_usb_audio_available {
...
mod tests {
    ...
    fn mo_call_reaches_active_with_negotiated_codec() {
        let mut machine = VoiceCallStateMachine::with_params(test_params());
        machine.mark_registration_ready();
        machine.queue_mo_call(VoiceLegKind::Vowifi);
        machine.submit_invite(2);
        machine.accept_provisional(180);
    ...
    fn dry_run_snapshot_walks_full_call() {
        let snap = build_dry_run_voice_snapshot_with_params(test_params());
        assert_eq!(snap.profile_id, "test_profile");
    ...
    fn leg_selection_prefers_vowifi_then_carrier() {
        assert_eq!(
            select_voice_leg_with_params(&test_params(), true, true),
            VoiceLegKind::Vowifi
        );
        // When VoWiFi not ready, fall back to carrier only if USB audio present.
        assert_eq!(
            select_voice_leg_with_params(&test_params(), false, true),
            if test_params().carrier_fallback_enabled {
                VoiceLegKind::Carrier
            } else {
                VoiceLegKind::None
            }
        ...
        assert_eq!(
            select_voice_leg_with_params(&test_params(), false, false),
            VoiceLegKind::None
        );
    }
}
