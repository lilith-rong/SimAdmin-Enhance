...
> **历史资料警告（2026-08-06）**：本文记录旧单卡实现与逆向背景，不再是当前配置/API 的权威来源。当前 VoLTE、VoWiFi、语音、ViLTE、APN、短信路径和 SIP Trunk 均必须通过 `line_id` 配置；文中的全局 JSON 字段和 `/api/volte/control` 等旧接口已删除，启动时不会迁移为线路配置。
...
已验证的 REGISTER 基线只在 `Contact` 中通过 `+g.3gpp.smsip` 声明 SMS 能力；
`P-Preferred-Service`、`Accept-Contact` 和 `Accept: application/vnd.3gpp.sms`
属于后续 SMS `MESSAGE`，不应加入 REGISTER。否则会扩大运营商 P-CSCF 对初始请求的
解析面，且与历史真机成功实现不一致。
...
> 详细计划与决策见本目录 `VOLTE_beta2_IDA逆向差异与后续修改指导.md`（保留，作为实施指南）。
...
## 14. 文档整合记录（2026-08-09）

为减少 SimAdmin 目录下的 md 文档数量，本文档继续作为 VoLTE 逆向与重构的**单一权威参考**；详细 IDA 实施指南见 `VOLTE_beta2_IDA逆向差异与后续修改指导.md`。本次整合/删除的历史文档：

| 已删除文档 | 原内容去向 |
|---|---|
| `VOLTE_beta2对齐重构计划.md` | 已并入本文 §3（IDA beta2 结论）与 §12（按 beta2 对齐的重构落地状态） |
| `VOLTE_secondary_ims_对齐计划.md` | QMI 端点能力与 IMS 承载路径结论并入 §5/§6 |
| `VOLTE_改进记录与待办.md` | 历史记录；唯一可用的"下一步验证顺序"见下方附录 A |
| `VOWIFI_VOLTE_四点改造计划.md` | 四点方案已被 line_id 线路化架构取代（原文自 2026-08-06 起标注废弃） |

### 附录 A：历史"下一步验证顺序"（自 `VOLTE_改进记录与待办.md` 保留）

1. 把 SIM 插入已开通 VoLTE 的手机拨打测试，确认号码开通状态；
2. 换已知开通 VoLTE 的卡，验证 `mmcli --create-bearer=apn=ims,ip-type=ipv4v6` 能建 bearer 并拿到 P-CSCF；
3. 连接成功后观察 Web UI attempt 字段（at_cid/qmi_device/bearer_path/interface/pcscf）、`ip_family_preference` 是否生效、AT 探测失败时能否靠 bearer PCO 拿 P-CSCF（日志 `continuing_with_bearer_pco`）。

> 恢复方式：上述文档均在 SimAdmin git 仓库历史中，可用 `git log -- <file>` / `git show HEAD:<path>` 取回。
...
- **工作计划**：见本文 §12（按 beta2 对齐的重构落地状态）与 `VOLTE_beta2_IDA逆向差异与后续修改指导.md` §6（建议实施阶段）。
