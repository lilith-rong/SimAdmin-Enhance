...
> **历史记录警告（2026-08-06）**：本文保留阶段性设计和旧基线，仅用于追溯。当前运行配置以按 `line_id` 的线路配置/API 为准；旧全局连接、语音、APN、路径策略和 Trunk 配置已删除且不做兼容迁移。CS 基带通话仍是逐线路控制，但不属于 Asterisk Trunk 的 IMS 语音路径。
